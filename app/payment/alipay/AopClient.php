...
class AopClient {
	...
	protected function sign($data, $signType = "RSA") {
		if($this->checkEmpty($this->rsaPrivateKeyFilePath)){
			$res = $this->getPrivateKeyResource($this->rsaPrivateKey);
		}else {
			$res = openssl_pkey_get_private(file_get_contents($this->rsaPrivateKeyFilePath));
		}
	...
	/**
	 * 将商户私钥（兼容带 PEM 头、PKCS#1/PKCS#8 纯 base64）解析为 openssl 私钥资源
	 */
	...
	protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
		error_log("[wm-alipay-sdk] method={$apiName} errorCode={$errorCode} requestUrl={$requestUrl} response=" . str_replace("\n", "", $responseTxt));
	}
	...
	*/
	...
	public function rsaCheckV2($params, $rsaPublicKeyFilePath, $signType='RSA') {
		$sign = $params['sign'];
		$params['sign'] = null;
		// 支付宝异步通知验签规则：剔除 sign 与 sign_type 后再拼接待验签字符串
		$params['sign_type'] = null;
		return $this->verify($this->getSignContent($params), $sign, $rsaPublicKeyFilePath, $signType);
	}
	...
	function verify($data, $sign, $rsaPublicKeyFilePath, $signType = 'RSA') {
		...
		if($this->checkEmpty($this->alipayPublicKey)){
			...
			$res = $this->getPublicKeyResource($this->alipayrsaPublicKey);
		}else {
			//读取公钥文件
			$res = openssl_pkey_get_public(file_get_contents($rsaPublicKeyFilePath));
		}
	...
	/**
	 * 将支付宝公钥（兼容带 PEM 头、X.509/RSA 纯 base64）解析为 openssl 公钥资源
	 */
	protected function getPublicKeyResource($key) {
		...
		if ($res === false) {
			$res = openssl_pkey_get_public("-----BEGIN RSA PUBLIC KEY-----\n" . wordwrap($body, 64, "\n", true) . "\n-----END RSA PUBLIC KEY-----");
		}
		return $res;
	}
